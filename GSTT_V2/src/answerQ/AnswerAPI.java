...
public class AnswerAPI {
	...
	public String answerQuestion(String question) throws UnsupportedEncodingException, MalformedURLException {
		...
		try {
			...
			String parameters = "?appid=" + APIKey + "&query=" + URLEncoder.encode(question, "UTF-8")
					+ "&output=xml&includepodid=Result&format=plaintext";

			String request = URLTarget + "?appid=" + APIKey + "&input=" + URLEncoder.encode(question, "UTF-8");
			...
			// replaces {,},[,] with empty char, to ensure emotionparser working
			// correctly.
			...
			requestPlaintext = requestPlaintext.replaceAll("[^a-zA-Z0-9/-/,]+", " ");
